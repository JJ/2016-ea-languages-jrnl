...
<<setup, cache=FALSE, results=hide, echo=FALSE>>=
pkgs = c("ggplot2", "ggthemes", "RColorBrewer")
to.install <- pkgs[ ! pkgs %in% installed.packages() ]

if ( length(to.install) > 0 )
  install.packages( to.install, dependencies = TRUE, repos="http://cran.rstudio.com/")
...
\author{Juan-Julián Merelo-Guervós \and Israel
    Blancas-Álvarez \and Pedro A. Castillo \and Gustavo Romero \and Pablo Garc{\'ia}-S{\'a}nchez\inst{1} 
\and
...
\institute{CITIC and Computer Architecture and Technology Department\\
  ...
  Email: (jmerelo,iblancasa,pacv,gustavo,pablogarcia)@ugr.es \and
...
community has mostly exploited a few popular ones, avoiding other less
popular or emerging languages, especially if they are not
compiled. There are more papers focused on 
implementation details and data structures, but these are usually limited to
specific languages. 
...
\begin{keywords}
Bench\-marking, implementation of evolutionary algorithms, OneMax, genetic operators, programming languages.
\end{keywords}
...
\ref{sec:res} we 
will present the results of measuring the performance of several
dozens of 
...
languages and mainly tried to measure their scaling behaviour across %Pablo: behaviour: Springer->British English
...
\begin{table*}[h!tb]
    ...
    \begin{tabular}{l|c|l|l|l}
      ...
      \hline
      C & 4.8.2 & \url{http://git.io/v8T57} & Bit String & Compiled \\
      ...
      R & Actualizar esto & Actualizar esto & Bit Vector & Interpreted \\
          % there are 23 languages in this table - Pedro
      %Corrected the language counting stuff :-) - JJ
      \hline
      \end{tabular}
      \vspace{5mm}
...
with a free license. The Linux system that has been tested is {\tt
